            ...
            if table:
                for row in table:
                    if len(row) >= 4 and row[0].isdigit():
                        ...
                        data.append([no_fp if no_fp else "Tidak ditemukan", nama_penjual if nama_penjual else "Tidak ditemukan", nama_pembeli if nama_pembeli else "Tidak ditemukan", nama_barang, harga, unit, qty, tanggal_faktur])
    return data
...
def main_app():
    ...
    if uploaded_files:
        ...
        if all_data:
            df = pd.DataFrame(all_data, columns=["No FP", "Nama Penjual", "Nama Pembeli", "Nama Barang", "Harga", "Unit", "QTY", "Tanggal Faktur"])
            df.index = df.index + 1  

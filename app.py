...
def main_app():
    ...
    if username != "admin":
        last_upload_date = st.session_state.get("last_upload_date")
        if last_upload_date == today:
            st.error("Anda hanya dapat mengunggah 1 file per hari.")
            return
    
    uploaded_files = st.file_uploader("Upload Faktur Pajak (PDF, bisa lebih dari satu)", type=["pdf"], accept_multiple_files=True if username == "admin" else False)
